...
%token SUPER DCOPY SCOPY COMPLEX 
%token PRINTCOMP
%token CASE DEFAULT DO OD NEXT
%token '+' '-' '*' '/' '%' '=' '>' '<' '.'
%token ',' ';' '!' '(' ')' '[' ']' '{' '}'
%token '@' '$' '#' ':'
...
%nonassoc ELSE
%nonassoc '@' '$' '#'
%nonassoc PRINTCOMP CASE DEFAULT SUPER DCOPY SCOPY DO OD
...
DoStmt			:	DO DoSubStmt DoBranchList OD
					{
						$$.stmt = new Tree.DoStmt((Tree.DoSubStmt)$2.stmt, $3.dslist, $1.loc);
					}
				;
...
DoBranchList	:	NEXT DoSubStmt DoBranchList
					{
						...
						$$.dslist.add((Tree.DoSubStmt)$2.stmt);
...
Expr            :   LValue
				...
				|   '@' Expr
					{
						$$.expr = new Tree.Unary(Tree.GETREAL, $2.expr, $1.loc);
					}
				|   '$' Expr
					{
						$$.expr = new Tree.Unary(Tree.GETIMG, $2.expr, $1.loc);
					}
				|   '#' Expr
					{
				...
				|	CASE '(' Expr ')' '{' ACaseExprList DefaultExpr '}'
					{
						$$.expr = new Tree.CaseExpr($3.expr, $6.celist, $7.expr, $1.loc);
					}
				;

ACaseExprList	:	ACaseExprList ACaseExpr
					{
						$$.celist.add((Tree.ACaseExpr)$2.expr);
					}
				|	/* empty */
					{
						$$ = new SemValue();
						$$.celist = new ArrayList<Tree.ACaseExpr>();
					}
				;

ACaseExpr		:	Constant ':' Expr ';'
					{
						$$.expr = new Tree.ACaseExpr((Tree.Literal)$1.expr, $3.expr, $1.loc);
					}
				;
